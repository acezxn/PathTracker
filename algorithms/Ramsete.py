...
def run(robot, input_path):
    ...
    while closest() != len(path)-1:
        ...
        desired_linearVelocity = min(max(0.3*abs(e_y), 10), 1000)
        
        desired_angularVelocity = min(max(0.1*abs(e_theta), 0), 2*math.pi)
                
        
        k = 2 * zeta * math.sqrt(desired_angularVelocity * desired_angularVelocity + b * desired_linearVelocity * desired_linearVelocity)
        targetLinearVelocity = desired_linearVelocity * math.cos(e_theta) + k * e_y
        
        
        targetAngularVelocity = 0
        if (e_theta != 0):
            targetAngularVelocity = desired_angularVelocity + k * e_theta + (b*desired_linearVelocity*math.sin(e_theta)* e_x) / e_theta
        
        last_wheels = wheels
        
        leftWeel = targetLinearVelocity + targetAngularVelocity
        rightWheel = targetLinearVelocity - targetAngularVelocity
        if leftWeel > robot.maxVel:
            leftWeel = robot.maxVel
        elif leftWeel < -robot.maxVel:
            leftWeel = -robot.maxVel
            
        if rightWheel > robot.maxVel:
            rightWheel = robot.maxVel
        elif rightWheel < -robot.maxVel:
            rightWheel = -robot.maxVel
